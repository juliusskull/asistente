...
$db = Conectar();	
 ...
 $sql1="select * from  creador1 where id=$id";
                    $stm = $db->prepare($sql1);
                    $stm->execute();
...
$aa= json_decode($jsontxt);
$bb= $aa;
//print_r($bb[0]);
$atributos=[];
$tipos=[];

foreach ($bb as $clave=>$valor)
{
...
$direccion ="salida/$id";
deleteDirectory($direccion);
mkdir("$direccion");
if (!file_exists("$direccion/interfaces")) {
mkdir("$direccion/interfaces");
}
if (!file_exists("$direccion/services")) {
mkdir("$direccion/services");
}
if (!file_exists("$direccion/components")) {
    mkdir("$direccion/components");
}
if (!file_exists("$direccion/components/$titulo")) {
mkdir("$direccion/components/$titulo");
}

$fp = fopen("$direccion/interfaces/". $titulo.".interface.ts", "w");
fwrite($fp,getInterface($atributos,$tipos,$titulo));


$fp = fopen("$direccion/services/". $titulo.".service.ts", "w");
fwrite($fp,getService($titulo,$url_servicio, ucfirst($titulo)));

$fp = fopen("$direccion/components/$titulo/". $titulo.".component.ts", "w");
fwrite($fp,getComponet($titulo,$titulo,$titulo,$atributos,$pk));

$fp = fopen("$direccion/components/$titulo/". $titulo.".component.html", "w");
...
zip_creation("$direccion", "temp/$archivo");
